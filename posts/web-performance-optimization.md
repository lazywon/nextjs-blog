...
- 성능최적화를 로딩 최적화와 렌더링 최적화로 구분할 수 있다.

```
성능 최적화 = 렌더링 최적화 + 로딩 최적화
```

- 렌더링, 로딩 과정을 종합적으로 최적화 하면 웹 성능을 평가하는핵심적인 웹 지표`(LCP,FID,CLS)` 수치를 개선할 수 있다.
...
- 렌더링 차단 리소스란 브라우저의 렌더링을 막는 소스들로 일반적으로 **css와 js 파일**을 말한다. _모든 css와 js가 렌더링 차단 리소스인 것은 아니다._
...
- 사용하지 않는 css 제거하기
  ...
  - Unused css는 구글 크롬 라이트하우스를 통해 확인할 수 있다.
- 간결한 스타일 작성하기
...
- 애니메이션 구현 시 자바스크립트 api, 라이브러리 보다는 css를 통해 구현하는 것이 성능면에서 좋다.
...
- 최적화의 첫 번 째 단계는 블록 리소스를 최적화하는 것이다.
- CSS 최적화
  ...
  - CSS 미디어 쿼리 사용하기 - 특정 조건에서만 필요한 CSS 파일이 있을 경우, 해당 스타일을 사용하는 경우에만 로드할 수 있도록 `<script>` 태그 안에 `media` 속성을 명시하여 사용한다. 반응형 웹 제작시 유용하게 사용할 수 있다.
